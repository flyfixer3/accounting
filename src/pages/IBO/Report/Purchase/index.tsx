...
import SelectMultipleSupplier from 'src/components/select/SelectMultipleSupplier'
...
import { IReportPurchaseListResponse } from 'src/models/report.model'
import useReportPurchaseController from '../controller/useReportPurchaseController'
...
const ReportPurchase = () => {
  ...
  const {
    ...
    onHandleSupplierFilter,
    onHandleDownloadReport,
  } = useReportPurchaseController()

  const TABLE_COLUMNS: ColumnsType<IReportPurchaseListResponse> = [
    {
      title: 'Order ID',
      render: (text, record, index) => {
        return (
          <div
            ...
            onClick={() => {
              navigate(`/supplier-order/${record?.orderId}`)
            }}>
            {record?.orderId || ''}
          </div>
        )
      ...
      render: (text, record, index) => {
        return (
          <div
            ...
            style={{ textAlign: 'left' }}
            onClick={() => {
              navigate(`/supplier/detail/${record?.supplier?.id}`)
            }}>
    ...
    },
    {
      ...
      render: (text, record, index) => {
        return <StatusBadge status={record?.paymentStatus} />
      },
      align: 'center',
    },
    {
  ...
  useEffect(() => {
    onSetBreadcrumbs([{ title: 'Purchase Report', path: '/report/purchase' }])
    onRefetch()
  }, [])
  ...
  return (
    <>
      <Row gutter={[8, 16]}>
        <Col xs={24}>
          <div className="detail-title">Purchase Report</div>
        </Col>
        <Col xs={24} sm={8}>
          <RangePicker
            style={{ width: '100%' }}
        ...
        <Col xs={24} sm={6}>
          <SelectMultipleSupplier
            handleSelectedData={onHandleSupplierFilter}
            allowClear={true}
          />
        </Col>
        <Col xs={6} sm={4}>
      ...
      <Table
        ...
        data={data?.data}
        isAction={false}
        rowKey={(data: IReportPurchaseListResponse) => data?.id}
        identifier={'id'}
